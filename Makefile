...
# $Id: //depot/mainline/cmd/gzip-1.2.4/Makefile#7 $
...
DEFS =  -DHAVE_UNISTD_H=1 -DDIRENT=1 -DVOID_CLOSEDIR=1
LIBS := 
...
GENFILES =  README NEWS INSTALL configure.in configure COPYING \
...
installoldgnu: installdirs installbin installman

install:
	@mkdir -p $(INSTALL_STD_BIN); \
	ln -s -f `/bin/pwd`/gzip$X $(INSTALL_STD_BIN)
...
clean:
	...
	rm -f *.pg *.pgs *.toc *.tp *.tps *.vr *.vrs *.d
